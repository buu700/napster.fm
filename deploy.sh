...
js/closure-library/closure/bin/build/closurebuilder.py --root=js $namespaceArgs -n require.goog -n require.napster -n exports -n init --output_mode=compiled --compiler_jar=compiler.jar --compiler_flags="--compilation_level=ADVANCED_OPTIMIZATIONS" --output_file=js/napster.js
